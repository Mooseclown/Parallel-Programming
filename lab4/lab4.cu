...
#define SHARE_MEM_SIZE  49125
#define WARP_SZIE       32
#define BLOCK_DIM_X     WARP_SZIE
#define BLOCK_DIM_Y     4
#define SHARE_WIDTH     (BLOCK_DIM_X + 2 * xBound)
#define SHARE_HEIGHT    (BLOCK_DIM_Y + 2 * yBound)
...
extern __shared__ unsigned char ss[];
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < height; y += gridDim.y * blockDim.y) {
        for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < width; x += gridDim.x * blockDim.x) {
            ...
            for (int v = -yBound; v <= yBound; v += 2) {
                for (int u = -xBound; u <= xBound; u += 2) {
                    if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                        int s_x = (x + u) - blockIdx.x * BLOCK_DIM_X + 2;
                        int s_y = (y + v) - blockIdx.y * BLOCK_DIM_Y + 2;
                        
                        #pragma unroll 3
                        for (int w = 0; w < channels; ++w) {
                            ss[channels * (SHARE_WIDTH * s_y + s_x) + w] = s[channels * (width * (y + v) + (x + u)) + w];
                        }
                    }
                }
            }
                ...
                for (int v = -yBound; v <= yBound; ++v) {
                    for (int u = -xBound; u <= xBound; ++u) {
                        if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                            int s_x = (x + u) - blockIdx.x * BLOCK_DIM_X + 2;
                            int s_y = (y + v) - blockIdx.y * BLOCK_DIM_Y + 2;
                            const unsigned char R = ss[channels * (SHARE_WIDTH * s_y + s_x) + 2];
                            const unsigned char G = ss[channels * (SHARE_WIDTH * s_y + s_x) + 1];
                            const unsigned char B = ss[channels * (SHARE_WIDTH * s_y + s_x) + 0];
...
int main(int argc, char **argv) {
    ...
    dim3 num_threads(BLOCK_DIM_X, BLOCK_DIM_Y);
    dim3 num_blocks(width / BLOCK_DIM_X + 1, height / BLOCK_DIM_Y + 1);

    // launch cuda kernel 
    sobel <<<num_blocks, num_threads, (channels * SHARE_WIDTH * SHARE_HEIGHT * sizeof(unsigned char))>>> (dsrc, ddst, height, width, channels);
