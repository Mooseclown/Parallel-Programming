...
#define MASK_X 5
#define MASK_Y 5
#define MASK_SIZE MASK_N * MASK_X *MASK_Y
#define SCALE 8
...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned *height,
                       unsigned *width, unsigned *channels, int *mask) {
    int  x, y, i, v, u;
    int  R, G, B;
    double val[MASK_N*3] = {0.0};
    int adjustX, adjustY, xBound, yBound;
    __shared__ int s_mask[MASK_SIZE];
        ...
        for (v = -yBound; v < yBound + adjustY; ++v) {
            for (u = -xBound; u < xBound + adjustX; ++u) {
                if ((x + u) >= 0 && (x + u) < *width && y + v >= 0 && y + v < *height) {
                    ...
                    val[i*3+2] += R * mask[(i) * MASK_X * MASK_Y + (u + xBound) * MASK_X + (v + yBound)];
                    val[i*3+1] += G * mask[(i) * MASK_X * MASK_Y + (u + xBound) * MASK_X + (v + yBound)];
                    val[i*3+0] += B * mask[(i) * MASK_X * MASK_Y + (u + xBound) * MASK_X + (v + yBound)];
                }    
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    unsigned long png_size = height * width * channels * sizeof(unsigned char);
    unsigned char* host_t = (unsigned char*) malloc(png_size);
    ...
    cudaMalloc((void **)&d_s, png_size);
    cudaMalloc((void **)&d_t, png_size);
    ...
    cudaMemcpy(d_s, host_s, png_size, cudaMemcpyHostToDevice);
    ...
    dim3 blk(WARP_SIZE, WARP_SIZE);
    dim3 grid(ceil(width/WARP_SIZE), ceil(height/WARP_SIZE));
    sobel<<<grid, blk>>>(d_s, d_t, d_height, d_width, d_channels, d_mask);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, d_t, png_size, cudaMemcpyDeviceToHost);
